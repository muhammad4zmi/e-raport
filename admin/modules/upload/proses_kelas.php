...
$data = new Spreadsheet_Excel_Reader($_FILES['filekelas']['tmp_name']);
...
for ($i=2; $i<=$baris; $i++)
{
  ...
  $kd_kelas = $data->val($i, 2);
  $kelas = $data->val($i, 3);
  $nis = $data->val($i, 4);
 
  $cekdata =mysqli_query($link, "select nis from tbl_kelas where nis ='$nis'");
    ...
    $alert = "<div class='alert alert-dismissable alert-warning'>
     ...
     $query = "INSERT INTO tbl_kelas
      VALUES ('','$id_kelas', '$kd_kelas', '$kelas','$nis')";
